...
function [Xtraining_Array_RSRP, Ytraining_regression_double_RSRP, Xvalidation_RSRP, Yvalidation_regression_double_RSRP] = Data_Generation_Transformer(Training_set_ratio, SNR_Range, Num_of_frame_each_SNR, type)
...
Ytraining_regression_double_RSRP = zeros(size(Pilot_location, 1) * size(Pilot_location, 2), 2, 1, Training_set_ratio * Num_of_frame_each_SNR * size(SNR_Range, 2));
...
Yvalidation_regression_double_RSRP = zeros(size(Pilot_location, 1) * size(Pilot_location, 2), 2, 1, Num_of_frame_each_SNR * size(SNR_Range, 2) - Training_set_ratio * Num_of_frame_each_SNR * size(SNR_Range, 2));
...
H_Ref_Transformer = zeros(size(Pilot_location, 1), Num_of_pilot);

for Pilot_location_symbol = Pilot_location_symbols
    H_Ref_Transformer(:, Pilot_location_symbol == Pilot_location_symbols) = H_Ref(Pilot_location(:, Pilot_location_symbol == Pilot_location_symbols), Pilot_location_symbol);
end
...
if Frame <= fix(Training_set_ratio * Num_of_frame_each_SNR)
    ...
    Ytraining_regression_double_RSRP(:, 1, 1, Training_index) = reshape(real(H_Ref_Transformer), [], 1);
    Ytraining_regression_double_RSRP(:, 2, 1, Training_index) = reshape(imag(H_Ref_Transformer), [], 1);
else
    ...
    Yvalidation_regression_double_RSRP(:, 1, 1, Validation_index) = reshape(real(H_Ref_Transformer), [], 1);
    Yvalidation_regression_double_RSRP(:, 2, 1, Validation_index) = reshape(imag(H_Ref_Transformer), [], 1);
end
